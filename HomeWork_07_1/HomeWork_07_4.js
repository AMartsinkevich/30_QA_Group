...
const searchUser = function(date) {

    for (let key of users) {
        if (key["registrationDate"] == date) {
            console.log(key)
        }
    }
...
searchUser("09.10.2021")
searchUser("10.10.2021")
...
console.log(uniqueCards(jsonUsers))

const fs = require("fs")
 
fs.readFile(
    "./HomeWork_07_1/task2.json",
    "utf8",
    function( error, data ) {
        if (error) throw error
        uniqueCards(JSON.parse(data))
    }
)
...
function countStaff() {
    ...
    for (let enterprise of enterprises) {
        ...
        console.log(`${enterprise["name"]} (${(total = enterprise["departments"].reduce( (a, b) => a + b["employees_count"], 0)) == 0 ? "нет" : total} сотрудников)`)
        
        for (let department of enterprise.departments) {

            console.log(`- ${department["name"]} (${(total = department.employees_count) == 0 ? "нет" : total} сотрудников)`)
